...
export default function ClosedOrders() {
  ...
  const generateBill = (order) => {
    ...
    doc.setFontSize(14)
    doc.text('GOLD JEWELRY INVOICE', 105, 35, { align: 'center' })
    doc.setFontSize(10)
  ...
  const fetchClosedOrders = async () => {
    try {
      const response = await axios.get('/api/orders/closed', {
        headers: {
    ...
    <div>
      ...
      <div className="orders-list">
        ...
        {filteredOrders.length === 0 && (
          <p style={{textAlign: 'center', padding: '2rem'}}>No closed orders found</p>
        )}
      </div>
    </div>
  )
