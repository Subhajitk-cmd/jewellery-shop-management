...
export default function Reports() {
  ...
  const fetchAnalytics = async () => {
    try {
      const response = await axios.get(`/api/analytics?year=${selectedYear}&view=${viewType}`, {
        headers: {
  ...
  return (
    <div>
      <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem'}}>
        <h2 style={{color: '#333'}}>📊 Reports & Analytics</h2>
      ...
      <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2rem'}}>
        <SimpleChart 
          ...
          title={`${viewType === 'monthly' ? 'Monthly' : 'Yearly'} Sales (₹)`} 
          color="#28a745"
          prefix="₹"
        />
        ...
        <SimpleChart 
          ...
          title={`${viewType === 'monthly' ? 'Monthly' : 'Yearly'} Orders`} 
          color="#007bff"
        />
        
        <SimpleChart 
          data={analytics.monthlyLoans} 
          title={`${viewType === 'monthly' ? 'Monthly' : 'Yearly'} Approved Loans`} 
          color="#ffc107"
        />
        
        <SimpleChart 
          data={analytics.monthlyLoanAmount} 
          title={`${viewType === 'monthly' ? 'Monthly' : 'Yearly'} Loan Amount (₹)`} 
          color="#dc3545"
          prefix="₹"
        />
        ...
        <SimpleChart 
          ...
          title={`${viewType === 'monthly' ? 'Monthly Making Charge' : 'Yearly Actual Making Charge'} (₹)`} 
          color="#6f42c1"
          prefix="₹"
        />
      ...
      <div style={{
        ...
        <h3 style={{marginBottom: '1rem', color: '#333'}}>Summary Statistics</h3>
        <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem'}}>
          <div style={{textAlign: 'center', padding: '1rem', backgroundColor: '#f8f9fa', borderRadius: '8px'}}>
            <h4 style={{color: '#28a745', margin: '0 0 0.5rem 0'}}>Total Sales</h4>
            <p style={{fontSize: '1.5rem', fontWeight: 'bold', margin: 0}}>
              ₹{analytics.monthlySales.reduce((sum, item) => sum + item.value, 0).toLocaleString()}
            </p>
          </div>
          <div style={{textAlign: 'center', padding: '1rem', backgroundColor: '#f8f9fa', borderRadius: '8px'}}>
            <h4 style={{color: '#007bff', margin: '0 0 0.5rem 0'}}>Total Orders</h4>
            <p style={{fontSize: '1.5rem', fontWeight: 'bold', margin: 0}}>
              {analytics.monthlyOrders.reduce((sum, item) => sum + item.value, 0)}
            </p>
          </div>
          <div style={{textAlign: 'center', padding: '1rem', backgroundColor: '#f8f9fa', borderRadius: '8px'}}>
            <h4 style={{color: '#ffc107', margin: '0 0 0.5rem 0'}}>Total Loans</h4>
            <p style={{fontSize: '1.5rem', fontWeight: 'bold', margin: 0}}>
              {analytics.monthlyLoans.reduce((sum, item) => sum + item.value, 0)}
            </p>
          </div>
          <div style={{textAlign: 'center', padding: '1rem', backgroundColor: '#f8f9fa', borderRadius: '8px'}}>
            <h4 style={{color: '#dc3545', margin: '0 0 0.5rem 0'}}>Total Loan Amount</h4>
            <p style={{fontSize: '1.5rem', fontWeight: 'bold', margin: 0}}>
              ₹{analytics.monthlyLoanAmount.reduce((sum, item) => sum + item.value, 0).toLocaleString()}
            </p>
          </div>
          <div style={{textAlign: 'center', padding: '1rem', backgroundColor: '#f8f9fa', borderRadius: '8px'}}>
            <h4 style={{color: '#6f42c1', margin: '0 0 0.5rem 0'}}>Total {viewType === 'monthly' ? 'Making Charge' : 'Actual Making Charge'}</h4>
            <p style={{fontSize: '1.5rem', fontWeight: 'bold', margin: 0}}>
          ...
          {viewType === 'yearly' && analytics.monthlyTotalDue && analytics.monthlyTotalDue.length > 0 && (
            <div style={{textAlign: 'center', padding: '1rem', backgroundColor: '#f8f9fa', borderRadius: '8px'}}>
              <h4 style={{color: '#fd7e14', margin: '0 0 0.5rem 0'}}>Total Due Amount</h4>
              <p style={{fontSize: '1.5rem', fontWeight: 'bold', margin: 0}}>
