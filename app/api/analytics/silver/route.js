...
export async function GET(request) {
  try {
    ...
    // Silver sales calculation based on view type
    const monthlySales = await Promise.all(
      periods.map(async (p) => {
        ...
        let salesData
        if (viewType === 'yearly') {
          // Yearly: Use itemActualValue and dateOfDelivery for closed silver orders
          ...
          // Monthly: Use itemEstimatedValue and dateOfBooking for silver orders
          salesData = await orders.aggregate([
            {
              $match: {
                userId: decoded.userId,
    ...
    // Silver orders calculation based on view type
    const monthlyOrders = await Promise.all(
      periods.map(async (p) => {
        ...
        let orderData
        if (viewType === 'yearly') {
          // Yearly: Count closed silver orders by dateOfDelivery
          ...
          // Monthly: Count all silver orders by dateOfBooking
          orderData = await orders.aggregate([
            {
              $match: {
                userId: decoded.userId,
    ...
    // Silver loans calculation based on view type
    ...
    // Silver loan amount calculation based on view type - summation of loanAmount
    ...
    // Silver making charge calculation based on view type
    const monthlyMakingCharge = await Promise.all(
      periods.map(async (p) => {
        ...
        if (viewType === 'yearly') {
          // Yearly: Use actualMakingCharge from closed silver orders by dateOfDelivery
          ...
          // Monthly: Use makingCharge from all silver orders by dateOfBooking
          makingChargeData = await orders.aggregate([
            {
              $match: {
                userId: decoded.userId,
    ...
    console.error('Silver analytics error:', error)
