...
export default function DepositedGold() {
  ...
  useEffect(() => {
    if (statusFilter === 'all') {
      setFilteredLoans(goldLoans)
    } else {
      setFilteredLoans(goldLoans.filter(loan => loan.status === statusFilter))
    }
  }, [goldLoans, statusFilter])

  const fetchGoldLoans = async () => {
    try {
      const response = await axios.get('/api/goldloan/search', {
        headers: {
  ...
  const handleLoanClick = async (loan, action) => {
    ...
    // Fetch fresh loan data for both actions
    ...
    <div>
      ...
      <div className="orders-list">
        {filteredLoans.map(loan => (
          <div key={loan._id} className="order-item" style={{borderLeft: '4px solid #FFD700'}}>
        ...
        {filteredLoans.length === 0 && (
          <p style={{textAlign: 'center', padding: '2rem'}}>No gold loans found for selected status</p>
        )}
      </div>
    </div>
  )
